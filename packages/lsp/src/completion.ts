...
import { ParsedDocument, TYPE_LABELS } from '@axiomate/parser';
...
export function onCompletion(params: CompletionParams, doc: ParsedDocument, textDoc: TextDocument): CompletionItem[] {
  ...
  return doc.statements
    ...
    .map((s) => {
      const typeLabel = TYPE_LABELS[s.annotation] ?? s.annotation;
      const bodyPreview = s.body.length > 80 ? s.body.slice(0, 80) + '...' : s.body;
      return {
        label: s.id,
        kind: CompletionItemKind.Reference,
        detail: `[${typeLabel}] ${bodyPreview}`,
